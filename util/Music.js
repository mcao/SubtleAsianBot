  ...
  try {
    ...
    class Music {
      constructor(client, options) {
        ...
        this.deleteQueue = {
          enabled:
            options.deleteQueue == undefined
              ? true
              : options.deleteQueue &&
                typeof options.deleteQueue.enabled !== 'undefined'
              ? options.deleteQueue && options.deleteQueue.enabled
              : true,
          run: 'deleteQueueFunction',
          alt:
            (options && options.deleteQueue && options.deleteQueue.alt) || [],
          help:
            (options && options.deleteQueue && options.deleteQueue.help) ||
            'Delete and re-make an ongoing queue',
          name:
            (options && options.deleteQueue && options.deleteQueue.name) ||
            'deletequeue',
          usage:
            (options && options.deleteQueue && options.deleteQueue.usage) ||
            null,
          exclude: Boolean(
            options && options.deleteQueue && options.deleteQueue.exclude
          ),
          masked: 'deletequeue'
        };
    ...
    musicbot.playFunction = (msg, suffix, args, ignore) => {
      ...
      let vc = client.voice.connections.find(
        val => val.channel.guild.id == msg.member.guild.id
      );
      if (vc && vc.channel.id != msg.member.voice.channel.id)
        return msg.channel.send(
      ...
      if (searchstring.startsWith('http') && searchstring.includes('list=')) {
        ...
        ytpl(playid, { limit: musicbot.maxQueueSize }, function(err, playlist) {
          ...
          if (
            playlist.total_items >= musicbot.maxQueueSize &&
            musicbot.maxQueueSize != 0
          )
            return msg.channel.send(
              musicbot.note(
                'fail',
                `Too many videos to queue. A maximum of ` +
                  musicbot.maxQueueSize +
                  ` is allowed.`
              )
            );
          var index = 0;
        ...
        new Promise(async (resolve, reject) => {
          let result = await musicbot.searcher
            .search(searchstring, { type: 'video' })
            .catch(err => {
              var errorMsg = err.message;
              if (errorMsg.includes('"reason": "dailyLimitExceeded",')) {
                errorMsg = errorMsg.slice(
                  errorMsg.indexOf('Daily Limit Exceeded. ')
                );
                ...
                if (!ignore)
                  msg.channel.send(
                    musicbot.note(
                      'fail',
                      'Unable to complete playback! Google API quota exceeded!'
                    )
                  );
                return;
              } else {
                if (!ignore)
                  msg.channel.send(
                    musicbot.note(
                      'fail',
                      'Unknown error occurred! Playback could not be completed, check the logs for more details.'
                    )
                  );
      ...
      if (!suffix) {
        ...
        if (msg.channel.permissionsFor(msg.guild.me).has('EMBED_LINKS')) {
          ...
          if (command.usage && typeof command.usage == 'string')
            embed.addField(
              `Usage`,
              command.usage.replace(/{{prefix}})/g, prefix),
              musicbot.inlineEmbeds
            );
          embed.setColor(musicbot.embedColor);
          msg.channel.send({
    ...
    musicbot.skipFunction = (msg, suffix, args) => {
      ...
      if (
        voiceConnection &&
        voiceConnection.channel.id != msg.member.voice.channel.id
      )
        return msg.channel.send(
          musicbot.note('fail', `You must be in the same voice channel as me.`)
        );
    ...
    musicbot.deleteQueueFunction = async (msg, suffix, args) => {
      if (!musicbot.isAdmin(msg.member))
        return msg.channel.send(
          musicbot.note('fail', 'Only and Admin can do this.')
        );
      const voiceConnection = client.voice.connections.find(
        val => val.channel.guild.id == msg.guild.id
      );
      musicbot
        ...
        .then(() => {
          ...
          return msg.channel.send(
            musicbot.note('note', 'The queue should now be emptied.')
          );
        })
        .catch(async res => {
          ...
          msg.channel.send(
            musicbot.note('note', 'The queue should now be deleted.')
          );
        });
    };
        ...
        if (queue.songs.length > 11) {
          ...
          try {
            ...
            return msg.channel.send(
              msicbot.note(
                'fail',
                'Something went wrong mapping out the queue! Please delete the queue if this persists.'
              )
            );
          }
        }
    ...
    musicbot.searchFunction = (msg, suffix, args) => {
      ...
      if (musicbot.userSearching.has(us))
        return msg.channel.send(
          musicbot.note(
            'fail',
            `You already have a search on-going for \`${
              musicbot.userSearching.get(us).title
      ...
      new Promise((resolve, reject) => {
        ...
        if (!voiceConnection) {
          if (msg.member.voice.channel && msg.member.voice.channel.joinable) {
            ...
            !msg.member.voice.channel.joinable ||
